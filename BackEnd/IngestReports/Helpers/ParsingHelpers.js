...
------------------------------------------------------------------------------*/
// Function to extract the <html> section cleanly
export function extractHtml(htmlOrBlob) {
	//Convert the raw html to a string so we can use regex match on it
	const dataArray = String(htmlOrBlob).match(/<html[\s\S]*?<\/html>/i);
	// Return html block if found, else return the original input as a string
	return dataArray ? dataArray[0] : String(htmlOrBlob);
}
